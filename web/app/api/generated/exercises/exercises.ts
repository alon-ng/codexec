/**
 * Generated by orval v7.17.2 🍺
 * Do not edit manually.
 * Codim API
 * This is the Codim API server.
 * OpenAPI spec version: 1.0
 */
import { useMutation, useQuery } from "@tanstack/react-query";
import type {
  DataTag,
  DefinedInitialDataOptions,
  DefinedUseQueryResult,
  MutationFunction,
  QueryClient,
  QueryFunction,
  QueryKey,
  UndefinedInitialDataOptions,
  UseMutationOptions,
  UseMutationResult,
  UseQueryOptions,
  UseQueryResult,
} from "@tanstack/react-query";

import type {
  DbExercise,
  ErrorsErrorResponse,
  ExercisesCreateExerciseRequest,
  ExercisesIDRequest,
  ExercisesUpdateRequest,
  GetExercisesParams,
} from ".././model";

import { customInstance } from "../../../lib/axios";

type SecondParameter<T extends (...args: never) => unknown> = Parameters<T>[1];

/**
 * Get a paginated list of exercises, optionally filtered by lesson UUID
 * @summary List exercises
 */
export const getExercises = (
  params?: GetExercisesParams,
  options?: SecondParameter<typeof customInstance>,
  signal?: AbortSignal,
) => {
  return customInstance<DbExercise[]>(
    { url: `/exercises`, method: "GET", params, signal },
    options,
  );
};

export const getGetExercisesQueryKey = (params?: GetExercisesParams) => {
  return [`/exercises`, ...(params ? [params] : [])] as const;
};

export const getGetExercisesQueryOptions = <
  TData = Awaited<ReturnType<typeof getExercises>>,
  TError = ErrorsErrorResponse,
>(
  params?: GetExercisesParams,
  options?: {
    query?: Partial<
      UseQueryOptions<Awaited<ReturnType<typeof getExercises>>, TError, TData>
    >;
    request?: SecondParameter<typeof customInstance>;
  },
) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};

  const queryKey = queryOptions?.queryKey ?? getGetExercisesQueryKey(params);

  const queryFn: QueryFunction<Awaited<ReturnType<typeof getExercises>>> = ({
    signal,
  }) => getExercises(params, requestOptions, signal);

  return { queryKey, queryFn, ...queryOptions } as UseQueryOptions<
    Awaited<ReturnType<typeof getExercises>>,
    TError,
    TData
  > & { queryKey: DataTag<QueryKey, TData, TError> };
};

export type GetExercisesQueryResult = NonNullable<
  Awaited<ReturnType<typeof getExercises>>
>;
export type GetExercisesQueryError = ErrorsErrorResponse;

export function useGetExercises<
  TData = Awaited<ReturnType<typeof getExercises>>,
  TError = ErrorsErrorResponse,
>(
  params: undefined | GetExercisesParams,
  options: {
    query: Partial<
      UseQueryOptions<Awaited<ReturnType<typeof getExercises>>, TError, TData>
    > &
      Pick<
        DefinedInitialDataOptions<
          Awaited<ReturnType<typeof getExercises>>,
          TError,
          Awaited<ReturnType<typeof getExercises>>
        >,
        "initialData"
      >;
    request?: SecondParameter<typeof customInstance>;
  },
  queryClient?: QueryClient,
): DefinedUseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
};
export function useGetExercises<
  TData = Awaited<ReturnType<typeof getExercises>>,
  TError = ErrorsErrorResponse,
>(
  params?: GetExercisesParams,
  options?: {
    query?: Partial<
      UseQueryOptions<Awaited<ReturnType<typeof getExercises>>, TError, TData>
    > &
      Pick<
        UndefinedInitialDataOptions<
          Awaited<ReturnType<typeof getExercises>>,
          TError,
          Awaited<ReturnType<typeof getExercises>>
        >,
        "initialData"
      >;
    request?: SecondParameter<typeof customInstance>;
  },
  queryClient?: QueryClient,
): UseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
};
export function useGetExercises<
  TData = Awaited<ReturnType<typeof getExercises>>,
  TError = ErrorsErrorResponse,
>(
  params?: GetExercisesParams,
  options?: {
    query?: Partial<
      UseQueryOptions<Awaited<ReturnType<typeof getExercises>>, TError, TData>
    >;
    request?: SecondParameter<typeof customInstance>;
  },
  queryClient?: QueryClient,
): UseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
};
/**
 * @summary List exercises
 */

export function useGetExercises<
  TData = Awaited<ReturnType<typeof getExercises>>,
  TError = ErrorsErrorResponse,
>(
  params?: GetExercisesParams,
  options?: {
    query?: Partial<
      UseQueryOptions<Awaited<ReturnType<typeof getExercises>>, TError, TData>
    >;
    request?: SecondParameter<typeof customInstance>;
  },
  queryClient?: QueryClient,
): UseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
} {
  const queryOptions = getGetExercisesQueryOptions(params, options);

  const query = useQuery(queryOptions, queryClient) as UseQueryResult<
    TData,
    TError
  > & { queryKey: DataTag<QueryKey, TData, TError> };

  query.queryKey = queryOptions.queryKey;

  return query;
}

/**
 * Create a new exercise
 * @summary Create a new exercise
 */
export const postExercisesCreate = (
  exercisesCreateExerciseRequest: ExercisesCreateExerciseRequest,
  options?: SecondParameter<typeof customInstance>,
  signal?: AbortSignal,
) => {
  return customInstance<DbExercise>(
    {
      url: `/exercises/create`,
      method: "POST",
      headers: { "Content-Type": "application/json" },
      data: exercisesCreateExerciseRequest,
      signal,
    },
    options,
  );
};

export const getPostExercisesCreateMutationOptions = <
  TError = ErrorsErrorResponse,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof postExercisesCreate>>,
    TError,
    { data: ExercisesCreateExerciseRequest },
    TContext
  >;
  request?: SecondParameter<typeof customInstance>;
}): UseMutationOptions<
  Awaited<ReturnType<typeof postExercisesCreate>>,
  TError,
  { data: ExercisesCreateExerciseRequest },
  TContext
> => {
  const mutationKey = ["postExercisesCreate"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey }, request: undefined };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof postExercisesCreate>>,
    { data: ExercisesCreateExerciseRequest }
  > = (props) => {
    const { data } = props ?? {};

    return postExercisesCreate(data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type PostExercisesCreateMutationResult = NonNullable<
  Awaited<ReturnType<typeof postExercisesCreate>>
>;
export type PostExercisesCreateMutationBody = ExercisesCreateExerciseRequest;
export type PostExercisesCreateMutationError = ErrorsErrorResponse;

/**
 * @summary Create a new exercise
 */
export const usePostExercisesCreate = <
  TError = ErrorsErrorResponse,
  TContext = unknown,
>(
  options?: {
    mutation?: UseMutationOptions<
      Awaited<ReturnType<typeof postExercisesCreate>>,
      TError,
      { data: ExercisesCreateExerciseRequest },
      TContext
    >;
    request?: SecondParameter<typeof customInstance>;
  },
  queryClient?: QueryClient,
): UseMutationResult<
  Awaited<ReturnType<typeof postExercisesCreate>>,
  TError,
  { data: ExercisesCreateExerciseRequest },
  TContext
> => {
  const mutationOptions = getPostExercisesCreateMutationOptions(options);

  return useMutation(mutationOptions, queryClient);
};
/**
 * Soft delete an exercise by UUID (sets deleted_at timestamp)
 * @summary Delete an exercise
 */
export const deleteExercisesDeleteUuid = (
  uuid: string,
  options?: SecondParameter<typeof customInstance>,
) => {
  return customInstance<string>(
    { url: `/exercises/delete/${uuid}`, method: "DELETE" },
    options,
  );
};

export const getDeleteExercisesDeleteUuidMutationOptions = <
  TError = ErrorsErrorResponse,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof deleteExercisesDeleteUuid>>,
    TError,
    { uuid: string },
    TContext
  >;
  request?: SecondParameter<typeof customInstance>;
}): UseMutationOptions<
  Awaited<ReturnType<typeof deleteExercisesDeleteUuid>>,
  TError,
  { uuid: string },
  TContext
> => {
  const mutationKey = ["deleteExercisesDeleteUuid"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey }, request: undefined };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof deleteExercisesDeleteUuid>>,
    { uuid: string }
  > = (props) => {
    const { uuid } = props ?? {};

    return deleteExercisesDeleteUuid(uuid, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type DeleteExercisesDeleteUuidMutationResult = NonNullable<
  Awaited<ReturnType<typeof deleteExercisesDeleteUuid>>
>;

export type DeleteExercisesDeleteUuidMutationError = ErrorsErrorResponse;

/**
 * @summary Delete an exercise
 */
export const useDeleteExercisesDeleteUuid = <
  TError = ErrorsErrorResponse,
  TContext = unknown,
>(
  options?: {
    mutation?: UseMutationOptions<
      Awaited<ReturnType<typeof deleteExercisesDeleteUuid>>,
      TError,
      { uuid: string },
      TContext
    >;
    request?: SecondParameter<typeof customInstance>;
  },
  queryClient?: QueryClient,
): UseMutationResult<
  Awaited<ReturnType<typeof deleteExercisesDeleteUuid>>,
  TError,
  { uuid: string },
  TContext
> => {
  const mutationOptions = getDeleteExercisesDeleteUuidMutationOptions(options);

  return useMutation(mutationOptions, queryClient);
};
/**
 * Restore a soft-deleted exercise by UUID (sets deleted_at to NULL)
 * @summary Restore a deleted exercise
 */
export const postExercisesRestore = (
  exercisesIDRequest: ExercisesIDRequest,
  options?: SecondParameter<typeof customInstance>,
  signal?: AbortSignal,
) => {
  return customInstance<string>(
    {
      url: `/exercises/restore`,
      method: "POST",
      headers: { "Content-Type": "application/json" },
      data: exercisesIDRequest,
      signal,
    },
    options,
  );
};

export const getPostExercisesRestoreMutationOptions = <
  TError = ErrorsErrorResponse,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof postExercisesRestore>>,
    TError,
    { data: ExercisesIDRequest },
    TContext
  >;
  request?: SecondParameter<typeof customInstance>;
}): UseMutationOptions<
  Awaited<ReturnType<typeof postExercisesRestore>>,
  TError,
  { data: ExercisesIDRequest },
  TContext
> => {
  const mutationKey = ["postExercisesRestore"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey }, request: undefined };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof postExercisesRestore>>,
    { data: ExercisesIDRequest }
  > = (props) => {
    const { data } = props ?? {};

    return postExercisesRestore(data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type PostExercisesRestoreMutationResult = NonNullable<
  Awaited<ReturnType<typeof postExercisesRestore>>
>;
export type PostExercisesRestoreMutationBody = ExercisesIDRequest;
export type PostExercisesRestoreMutationError = ErrorsErrorResponse;

/**
 * @summary Restore a deleted exercise
 */
export const usePostExercisesRestore = <
  TError = ErrorsErrorResponse,
  TContext = unknown,
>(
  options?: {
    mutation?: UseMutationOptions<
      Awaited<ReturnType<typeof postExercisesRestore>>,
      TError,
      { data: ExercisesIDRequest },
      TContext
    >;
    request?: SecondParameter<typeof customInstance>;
  },
  queryClient?: QueryClient,
): UseMutationResult<
  Awaited<ReturnType<typeof postExercisesRestore>>,
  TError,
  { data: ExercisesIDRequest },
  TContext
> => {
  const mutationOptions = getPostExercisesRestoreMutationOptions(options);

  return useMutation(mutationOptions, queryClient);
};
/**
 * Update an existing exercise by UUID
 * @summary Update an exercise
 */
export const putExercisesUpdate = (
  exercisesUpdateRequest: ExercisesUpdateRequest,
  options?: SecondParameter<typeof customInstance>,
) => {
  return customInstance<DbExercise>(
    {
      url: `/exercises/update`,
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      data: exercisesUpdateRequest,
    },
    options,
  );
};

export const getPutExercisesUpdateMutationOptions = <
  TError = ErrorsErrorResponse,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof putExercisesUpdate>>,
    TError,
    { data: ExercisesUpdateRequest },
    TContext
  >;
  request?: SecondParameter<typeof customInstance>;
}): UseMutationOptions<
  Awaited<ReturnType<typeof putExercisesUpdate>>,
  TError,
  { data: ExercisesUpdateRequest },
  TContext
> => {
  const mutationKey = ["putExercisesUpdate"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey }, request: undefined };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof putExercisesUpdate>>,
    { data: ExercisesUpdateRequest }
  > = (props) => {
    const { data } = props ?? {};

    return putExercisesUpdate(data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type PutExercisesUpdateMutationResult = NonNullable<
  Awaited<ReturnType<typeof putExercisesUpdate>>
>;
export type PutExercisesUpdateMutationBody = ExercisesUpdateRequest;
export type PutExercisesUpdateMutationError = ErrorsErrorResponse;

/**
 * @summary Update an exercise
 */
export const usePutExercisesUpdate = <
  TError = ErrorsErrorResponse,
  TContext = unknown,
>(
  options?: {
    mutation?: UseMutationOptions<
      Awaited<ReturnType<typeof putExercisesUpdate>>,
      TError,
      { data: ExercisesUpdateRequest },
      TContext
    >;
    request?: SecondParameter<typeof customInstance>;
  },
  queryClient?: QueryClient,
): UseMutationResult<
  Awaited<ReturnType<typeof putExercisesUpdate>>,
  TError,
  { data: ExercisesUpdateRequest },
  TContext
> => {
  const mutationOptions = getPutExercisesUpdateMutationOptions(options);

  return useMutation(mutationOptions, queryClient);
};
/**
 * Get a single exercise by UUID
 * @summary Get an exercise by UUID
 */
export const getExercisesUuid = (
  uuid: string,
  options?: SecondParameter<typeof customInstance>,
  signal?: AbortSignal,
) => {
  return customInstance<DbExercise>(
    { url: `/exercises/${uuid}`, method: "GET", signal },
    options,
  );
};

export const getGetExercisesUuidQueryKey = (uuid?: string) => {
  return [`/exercises/${uuid}`] as const;
};

export const getGetExercisesUuidQueryOptions = <
  TData = Awaited<ReturnType<typeof getExercisesUuid>>,
  TError = ErrorsErrorResponse,
>(
  uuid: string,
  options?: {
    query?: Partial<
      UseQueryOptions<
        Awaited<ReturnType<typeof getExercisesUuid>>,
        TError,
        TData
      >
    >;
    request?: SecondParameter<typeof customInstance>;
  },
) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};

  const queryKey = queryOptions?.queryKey ?? getGetExercisesUuidQueryKey(uuid);

  const queryFn: QueryFunction<
    Awaited<ReturnType<typeof getExercisesUuid>>
  > = ({ signal }) => getExercisesUuid(uuid, requestOptions, signal);

  return {
    queryKey,
    queryFn,
    enabled: !!uuid,
    ...queryOptions,
  } as UseQueryOptions<
    Awaited<ReturnType<typeof getExercisesUuid>>,
    TError,
    TData
  > & { queryKey: DataTag<QueryKey, TData, TError> };
};

export type GetExercisesUuidQueryResult = NonNullable<
  Awaited<ReturnType<typeof getExercisesUuid>>
>;
export type GetExercisesUuidQueryError = ErrorsErrorResponse;

export function useGetExercisesUuid<
  TData = Awaited<ReturnType<typeof getExercisesUuid>>,
  TError = ErrorsErrorResponse,
>(
  uuid: string,
  options: {
    query: Partial<
      UseQueryOptions<
        Awaited<ReturnType<typeof getExercisesUuid>>,
        TError,
        TData
      >
    > &
      Pick<
        DefinedInitialDataOptions<
          Awaited<ReturnType<typeof getExercisesUuid>>,
          TError,
          Awaited<ReturnType<typeof getExercisesUuid>>
        >,
        "initialData"
      >;
    request?: SecondParameter<typeof customInstance>;
  },
  queryClient?: QueryClient,
): DefinedUseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
};
export function useGetExercisesUuid<
  TData = Awaited<ReturnType<typeof getExercisesUuid>>,
  TError = ErrorsErrorResponse,
>(
  uuid: string,
  options?: {
    query?: Partial<
      UseQueryOptions<
        Awaited<ReturnType<typeof getExercisesUuid>>,
        TError,
        TData
      >
    > &
      Pick<
        UndefinedInitialDataOptions<
          Awaited<ReturnType<typeof getExercisesUuid>>,
          TError,
          Awaited<ReturnType<typeof getExercisesUuid>>
        >,
        "initialData"
      >;
    request?: SecondParameter<typeof customInstance>;
  },
  queryClient?: QueryClient,
): UseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
};
export function useGetExercisesUuid<
  TData = Awaited<ReturnType<typeof getExercisesUuid>>,
  TError = ErrorsErrorResponse,
>(
  uuid: string,
  options?: {
    query?: Partial<
      UseQueryOptions<
        Awaited<ReturnType<typeof getExercisesUuid>>,
        TError,
        TData
      >
    >;
    request?: SecondParameter<typeof customInstance>;
  },
  queryClient?: QueryClient,
): UseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
};
/**
 * @summary Get an exercise by UUID
 */

export function useGetExercisesUuid<
  TData = Awaited<ReturnType<typeof getExercisesUuid>>,
  TError = ErrorsErrorResponse,
>(
  uuid: string,
  options?: {
    query?: Partial<
      UseQueryOptions<
        Awaited<ReturnType<typeof getExercisesUuid>>,
        TError,
        TData
      >
    >;
    request?: SecondParameter<typeof customInstance>;
  },
  queryClient?: QueryClient,
): UseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
} {
  const queryOptions = getGetExercisesUuidQueryOptions(uuid, options);

  const query = useQuery(queryOptions, queryClient) as UseQueryResult<
    TData,
    TError
  > & { queryKey: DataTag<QueryKey, TData, TError> };

  query.queryKey = queryOptions.queryKey;

  return query;
}
